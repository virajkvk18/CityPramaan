...
export function AnimatedCityMap({ reports, city }: AnimatedCityMapProps) {
  ...
  return (
    <div className="glass-panel relative min-h-[620px] flex-1 overflow-hidden rounded-md border border-white/10 bg-[#101014] shadow-[0_0_36px_rgba(0,0,0,0.55)]">
      ...
      <div className="cp-map-radar pointer-events-none absolute left-1/2 top-1/2 z-10 h-[360px] w-[360px] -translate-x-1/2 -translate-y-1/2 rounded-full" />
      ...
      <div className="absolute right-5 top-5 z-30 max-w-xs rounded-md border border-[#ffc08d]/30 bg-black/65 p-3 backdrop-blur-md">
      ...
      <div className="absolute inset-0 z-20">
        {visibleReports.map((report, index) => {
          ...
          return (
            ...
            >
              <div className={`h-4 w-4 rounded-full ${tone.dot} ${tone.pulse}`} />
              <div className={`h-9 w-px bg-gradient-to-b ${tone.stem} to-transparent`} />
...
function SignalPill({ label, value, tone }: { label: string; value: string; tone: string }) {
  return (
    <div className="rounded-md border border-white/10 bg-black/55 px-3 py-2 backdrop-blur-md">
      <p className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-[#a38d7c]">{label}</p>
